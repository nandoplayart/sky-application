...
validateUser(email,senha,user){
    let senhaDecrypt = crypto.AES.decrypt(user.senha,process.env.SALT_ROUNDS).toString(crypto.enc.Utf8);
 ...
 updateLastAccess =async (user) =>{
     ...
     await User.updateOne({_id: user._id},{$set:{ultimo_login:user.ultimo_login }})
 }
 ...
 getUser = async (id)=>{
   let user = await  User.findById(id,'_id nome email telefones token data_criacao data_atualizacao ultimo_login');
   return user;
 }

 getAllUsers = async()=>{
     return await User.find({},'_id nome email telefones token data_criacao data_atualizacao ultimo_login');
 }
